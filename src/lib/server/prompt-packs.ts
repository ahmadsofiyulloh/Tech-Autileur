...
const MOCK_SINGLE_FRAME_FIRST_FRAME_INSTRUCTION =
...
const MOCK_SINGLE_FRAME_I2V_INSTRUCTION =
...
function buildMockI2IMustAvoid(frame: "first_frame" | "last_frame") {
  return [
    ...
    frame === "first_frame" ? "Do not output multiple image files, a multi-image layout, a numbered sequence, or a video." : "Do not introduce @character, @environment, or product reference images again; use @firstframe only for legacy compatibility.",
...
function buildMockTimeline(clipKey: "clip_1" | "clip_2") {
  ...
  return PROMPT_PACK_I2V_TIMELINE_WINDOWS.map((time, index) => {
    const action =
      index === 0
        ? "Start from @firstframe as the opening beat with established character, outfit, lighting, and room."
        : index === 1
          ? `Continue the same shot for ${objective}; add a subtle posture shift without changing product details.`
          : index === 2
            ? "Use slow controlled camera movement and slight parallax from the same first-frame setup; keep the polo graphic readable."
            : "Resolve from the same first-frame setup with the same identity, product, and background; @lastframe remains legacy compatibility only.";
...
function buildI2IPrompts(context: MockPromptContext): PromptPackGenerationOutput["i2i_prompts"] {
  const { promptPack, product, latestAnchor, promptContext } = context;
  ...
  return {
    clip_1: {
      ...
      first_frame: {
        ...
        prompt_text: compactText(
          `${product.product_name} ${promptPack.prompt_code} v${promptPack.version}. ${MOCK_SINGLE_FRAME_FIRST_FRAME_INSTRUCTION} Build the ${buildMockClipObjective("clip_1")} in the locked industrial warm-orange room. ${continuityInstruction}`,
          420,
        ),
    ...
    clip_2: {
      ...
      first_frame: {
        ...
        image_inputs: ["@character", "@environment", productMention],
        prompt_text: compactText(
          `${product.product_name} ${promptPack.prompt_code} v${promptPack.version}. ${MOCK_SINGLE_FRAME_FIRST_FRAME_INSTRUCTION} Build the ${buildMockClipObjective("clip_2")} in the same locked industrial warm-orange room. PRODUCT stays primary and readable.`,
          420,
        ),
        must_keep: buildMockI2IMustKeep({
          clipKey: "clip_2",
...
function buildPromptPackRepairPrompt(
...
async function listPromptPackRepairKeySelections(
  ...
  const selections: { key: GeminiSelectedKey; secret: string; role: string }[] = [];
  ...
  if (!seenKeyIds.has(fallbackSelection.id) && !excludedKeyIds.has(fallbackSelection.id)) {
    const secretResult = await readGeminiSecretForKey(context.serviceClient, context.user.id, fallbackSelection.id);
    sawSecretDecryptionFailure ||= secretResult.decryptFailed;
    ...
    if (secretResult.secret) {
      ...
      seenKeyIds.add(fallbackSelection.id);
    }
  }
...
async function repairPromptPackGenerationOutput(input: {
  ...
  const repairSelections = await listPromptPackRepairKeySelections(
    input.context,
    ...
    try {
      ...
      if (disposition.markKeyError) {
        ...
        await markGeminiKeyError({
          serviceClient: input.context.serviceClient,
          userId: input.context.user.id,
      ...
      if (isGeminiTransientFailure(error) || !disposition.retryableTask) {
        input.excludedQuotaGroups.add(getGeminiQuotaGroupKey(selection.key));
        continue;
      }
  ...
  throw lastError instanceof Error ? lastError : new Error("Unable to repair prompt-pack Gemini output.");
}
...
export async function runRealPromptPackTask(promptPackId: string, taskId: string) {
  ...
  const failedTask = await markTaskFailed(taskId, message, {
