import { cancelProductBulkImportJob } from "@/lib/server/product-bulk-import";
...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    const rateLimitResponse = rateLimitResponseForRequest(request, API_RATE_LIMITS.bulkImportJobCancel);
    if (rateLimitResponse) {
      return rateLimitResponse;
    }

    const { jobId } = await context.params;
    const snapshot = await cancelProductBulkImportJob(jobId);
